...
for($i=0;$i<16;$i++){
	for($j=0;$j<20;$j++){
		$n = bcmul($array[$j][$i],$array[$j][$i+1]);
		$n = bcmul($n,$array[$j][$i+2]);
		$n = bcmul($n,$array[$j][$i+3]);
		if($max<$n) $max = $n;
	}
}
//Le Fel
for($i=0;$i<16;$i++){
	for($j=0;$j<20;$j++){
		$n = bcmul($array[$i][$j],$array[$i+1][$j]);
		$n = bcmul($n,$array[$i+2][$j]);
		$n = bcmul($n,$array[$i+3][$j]);
		if($max<$n) $max = $n;
	}
}
...
for($i=0;$i<16;$i++){
	for($j=0;$j<16;$j++){
		$n = bcmul($array[$i][$j],$array[$i+1][$j+1]);
		$n = bcmul($n,$array[$i+2][$j+2]);
		$n = bcmul($n,$array[$i+3][$j+3]);
		if($max<$n) $max = $n;
	}
}
...
for($i=0;$i<16;$i++){
	for($j=0;$j<16;$j++){
		$n = bcmul($array[$i][$j+3],$array[$i+1][$j+2]);
		$n = bcmul($n,$array[$i+2][$j+1]);
		$n = bcmul($n,$array[$i+3][$j]);
		if($max<$n) $max = $n;
	}
}
